...
namespace CGAL {
  ...
  public:
    ...
    /*!
     * \brief adjust_normal_by_points_normal
     */
    void adjust_normal_by_points_normal(){
...
//          Vector_3 point_normal = this->normal(indices[i]);

//          if( (this->get_x(point_normal) * this->get_x(m_normal)
//               + this->get_y(point_normal) * this->get_y(m_normal)
//               + this->get_z(point_normal) * this->get_z(m_normal)) <0  ){
//              need_revese++;
//          }
//      }
//      std::cout<<"indices.size() : "<<indices.size()<<" need revese" <<std::endl;
//      if(need_revese < indices.size() -need_revese ){
//            m_normal = m_normal * -1;
//      }
...
//            < this->m_normal_threshold * CGAL::sqrt(this->sqlen(l_v))) {
        if (this->scalar_pdct(l_v, m_normal)
            < this->m_normal_threshold * CGAL::sqrt(this->sqlen(l_v))) {
...
//      return CGAL::abs(this->scalar_pdct(n, m_normal));
      return this->scalar_pdct(n, m_normal);
    } 
